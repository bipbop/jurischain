...
PHP_MINIT_FUNCTION(jurischain) {
  res_num = zend_register_list_destructors_ex(jurischain_res_dtor, NULL,
                                              LE_JURISCHAIN, module_number);
  return SUCCESS;
}
...
PHP_FUNCTION(jurischain_try) {
  ...
  RETURN_BOOL(jurischain_try((jurischain_ctx_t *)zend_fetch_resource_ex(
      challenge, LE_JURISCHAIN, res_num)));
}
...
PHP_FUNCTION(jurischain_verify) {
  ...
  RETURN_BOOL(jurischain_verify((jurischain_ctx_t *)zend_fetch_resource_ex(
      challenge, LE_JURISCHAIN, res_num)));
}
...
PHP_FUNCTION(jurischain_get) {
  ...
  jurischain_ctx_t *ctx = (jurischain_ctx_t *)zend_fetch_resource_ex(
      challenge, LE_JURISCHAIN, res_num);
  char str[(HASH_LEN * 2) + 1] = {
      0,
  };
  for (int i = 0; i < HASH_LEN; i++)
    sprintf(str + (i * 2), "%02hhX", ctx->seed[i]);
  RETURN_STRING(str);
}
