...
char *loading = "<div class='robot-captcha__marker'><div "
                "class='robot-captcha__checkbox--loading'></div></div>"
                "<div class='robot-captcha__text'>Aguarde enquanto validamos "
                "seu acesso.</span></div>";

char *completed = "<div class='robot-captcha__marker'><div "
                  "class='robot-captcha__checkbox--completed'></div></div>"
                  "<div class='robot-captcha__text'>Seu acesso foi validado "
                  "com sucesso.</span></div>";

EM_JS(void, robotElement, (const char *content, int solved, char *challenge), {
  if (solved)
    document.dispatchEvent(
        new CustomEvent('robotCaptcha', {'detail' : UTF8ToString(challenge)}));
  var robotCaptchaElement = document.getElementById('robot-captcha');
  if (!robotCaptchaElement)
    return;
...
void EMSCRIPTEN_KEEPALIVE try_solve() {
  for (int i = 0; i < 15; i++) {
    ...
    for (int i = 0; i < HASH_LEN; i++)
      sprintf(challenge + (i * 2), "%02X", pow_i.seed[i]);
...
int main() {
  if (!emscripten_run_script_int(
          "!document || !document.robotCaptcha || typeof "
          "document.robotCaptcha.seed !== 'string' || typeof "
          "document.robotCaptcha.difficulty !== 'string'"))
    return 1;
  char *seed = emscripten_run_script_string("document.robotCaptcha.seed");
  uint8_t difficulty =
      emscripten_run_script_int("document.robotCaptcha.difficulty");
